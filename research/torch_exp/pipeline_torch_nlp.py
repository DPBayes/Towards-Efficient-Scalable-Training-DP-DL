...
def test_nlp(
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_loader):
            ...
            targets = inputs['labels'].detach().cpu().numpy()
    ...
    acc = correct_test / total_test if total_test > 0 else 0.0
...
def load_nlp_model(model_name):
    ...
    if model_name == 'distilbert/distilbert-base-uncased':
...
def load_nlp_data(dataset_name):
...
def tokenized_to_dataset(features):
...
def dataset_to_dataloader(train_dataset,test_dataset,logical_bs,test_bs, generator):
    train_dataloader = DataLoader(train_dataset, batch_size=logical_bs, 
        shuffle=True,
        num_workers=0,  # Important for Opacus
        generator=generator
        )
    test_dataloader = DataLoader(test_dataset,batch_size=10,shuffle=False,generator=generator)
...
def main_non_distributed(args):
    ...
    train_set, test_set = load_nlp_data(args.dataset)

    args.bs = len(train_set) // 2

    print('logical batch size',args.bs, flush=True)

    train_tokenized = tokenize_data(train_set, tokenizer, args.max_length)
    test_tokenized = tokenize_data(test_set, tokenizer, args.max_length)

    train_dataset = tokenized_to_dataset(train_tokenized)
    test_dataset = tokenized_to_dataset(test_tokenized)

    train_loader, test_loader = dataset_to_dataloader(
        train_dataset,
        test_dataset,
        args.bs,
        args.phy_bs,
    ...
    if lib == "opacus":
        ...
        print("Opacus model type", type(model))
        print("Opacus optimizer type", type(optimizer))
        print("Opacus loader type", type(train_loader),"train loader len",len(train_loader))
    elif lib != "non":
        train_loader = privatize_dataloader(
            train_loader, dist
        )  # The BatchMemoryManager of Opacus does this step. Since here we are implementing our own, we have to do this step explicitly before.
    ...
    throughs = np.zeros(args.epochs)
    total_thr = np.zeros(args.epochs)
    acc_wt = test_nlp(device, model, lib, test_loader, criterion, 0)
    print("Without training accuracy", acc_wt)
    for epoch in range(args.epochs):
        print("memory allocated ", torch.cuda.memory_allocated() / 1024**2, flush=True)
        if lib == "opacus":
            th, t_th = train_opacus_nlp(device, model, train_loader, optimizer, criterion, epoch, args.phy_bs)
            privacy_results = privacy_engine.get_epsilon(args.target_delta)  # type: ignore
            privacy_results = {"eps_rdp": privacy_results}
            print("Privacy results after training {}".format(privacy_results), flush=True)
        elif lib == "non":
            th, t_th = train_non_private_nlp(
                device, model, lib, train_loader, optimizer, criterion, epoch, args.phy_bs, n_acc_steps
            )
        else:
            th, t_th = train_efficient_gradient_clipping(
        ...
        throughs[epoch] = th
        total_thr[epoch] = t_th
        test_accs[epoch] = test_nlp(device, model, lib, test_loader, criterion, epoch)

        torch.cuda.empty_cache()

    print("--- Finished training ---", flush=True)
    acc = test_nlp(device, model, lib, test_loader, criterion, epoch)
    thr = np.mean(throughs)
    # acc = test_accs[-1]
    t_th = np.mean(total_thr)
